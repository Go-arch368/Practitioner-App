...
const ProviderSearchPractitioner: React.FC = () => {
  ...
  const searchProvider = async (e: React.FormEvent) => {
    ...
    console.log('Practitioner Search Request Body =>', JSON.stringify(updatedFormData));
  ...
  useEffect(() => {
    ...
    if (status === ReducerStates.failed) {
      ...
      setShowResult(false);
      console.error('Search Practitioners failed', error);
    } else if (status === ReducerStates.succeeded) {
      dispatch(resetPractitionerStatus());
      setShowErrorPage(false);

      if (practitioners.length >= 1000) {
  ...
  }, [practitioners, status, error, fields, dispatch]);
                    ...
                    <StateSelectorForm
                     ref={stateSelectorRef}
                      ...
                      onSelectionChange={(selected) =>
                        field.setValue(selected.map((s: any) => s.value))
                      }
                    />
                  ) :(field.name==="Network")?(
                     <NetworkSelectorForm
                        ...
                        initialSelection={
                          Array.isArray(field.value)
                            ? field.value[0] // if array, take first element
                            : field.value || '' // otherwise just use the string
                        }
                        onSelectionChange={(selected) => {
                          if (selected) {
...
:
            ...
            <ExpandableComponent title="Advanced Search Options" searchPage="practitionerSearch">
              <span>TODO</span>
            </ExpandableComponent>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end mt-4">
          <button type="button" className="btn btn-secondary me-3" onClick={cancel}>
            {CANCEL}
          </button>
